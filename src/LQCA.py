...
def contains_main_pos(POS_tags):
    return any(tag in MAIN_TAGS for tag in POS_tags)
...
def lqca_pipeline(dataset_name, dataset_subset, context_field, output_dir):
    ...
    def process_item(item, item_idx):
        ...
        try:
            ...
            generator = books.pipeline(corpipe_tags_map, corpipe_zdeprels_map, False, CORPIPE_ARGS).ragged_batch(
                CORPIPE_ARGS.batch_size).prefetch(tf.data.AUTOTUNE)
        ...
        for sent_id, sent_mentions in enumerate(doc_mentions):
            ...
            for mention in sent_mentions:
                ...
                # Проверяем, видели ли мы уже это нормализованное упоминание в данном кластере (или упоминание нормализуется в пустоту)
                if not normalized_mention or normalized_mention in seen_normalized_mentions_in_cluster[cluster_id]:
